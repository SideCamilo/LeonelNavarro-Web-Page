...
/////////////////////////LEE CLIENTES
if ($_POST['id'] == 0) {
	$buscar = 'SELECT p . * , e.estado AS nombre_estado, z.zona AS nombre_zona FROM propiedades p, estados e, zonas z WHERE p.zona = z.id AND p.estado = e.id AND p.zona = z.id and publica = "N" ORDER BY p.descripcion ASC ';

}else{
$buscar = 'SELECT p.*, e.estado as nombre_estado, z.zona as nombre_zona FROM propiedades p, estados e, zonas z where p.zona = '.$_POST['id'].' and p.estado = e.id and p.zona = z.id and publica = "N" order by p.descripcion ASC';}
			...
			<div id="mycarousel" class="carousel slide" data-ride="carousel" style = "cursor:pointer;">
				<div class="carousel-inner">
					<div class="item active">
						<img style = "width:100%; height: 300px;" id = "" src = "../img/prop/'.$row_sql_buscar['imagen'].'" alt = "'.$row_sql_buscar['nombre'].'" onclick="inverr('.$row_sql_buscar['id'].')"/>
